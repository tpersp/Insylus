...
func (Plugin) Register(host pluginhost.Host) error {
	...
	if host.API().Enabled() {
		...
		host.API().HandleFunc("GET /api/homebox/assets/template", rt.handleAssetTemplate)
		host.API().HandleFunc("GET /api/homebox/self", rt.handleSelf)
		host.API().HandleFunc("GET /api/homebox/items", rt.handleItems)
		host.API().HandleFunc("POST /api/homebox/items", rt.handleCreateAsset)
		host.API().HandleFunc("GET /api/homebox/items/{id}", rt.handleItem)
		host.API().HandleFunc("PATCH /api/homebox/items/{id}", rt.handleUpdateAsset)
		host.API().HandleFunc("POST /api/homebox/assets", rt.handleCreateAsset)
		host.API().HandleFunc("PATCH /api/homebox/assets/{id}", rt.handleUpdateAsset)
